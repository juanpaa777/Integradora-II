...
function generateToken(user) {
  const payload = {
    ...
    roles: user.roles || ['user'], // Asigna roles por defecto si no existen
  };

  return jwt.sign(payload, 'JWT-1234567890', { expiresIn: '1h' });
}
...
function verifyToken(req, res, next) {
  ...
  jwt.verify(token, 'JWT-1234567890', (err, decoded) => {
    if (err) return res.status(500).send({ message: 'Fallo al autenticar el token.' });
  ...
  try {
    ...
    pool.query('SELECT * FROM Lector2 WHERE NombreUsuario = ?', [username], (err, lectorResults) => {
      ...
      if (lectorResults.length > 0) {
        ...
        if (bcrypt.compareSync(password, lector.Contrasena)) {
          const token = generateToken(lector);
          return res.status(200).send({ message: 'Login realizado exitosamente como lector', token });
        }
      }

      // Si no se encuentra en Lector2, verificar en Bibliotecario
      pool.query('SELECT * FROM Bibliotecario WHERE NombreUsuario = ?', [username], (err, bibliotecarioResults) => {
        ...
        if (bibliotecarioResults.length > 0) {
          ...
          if (bcrypt.compareSync(password, bibliotecario.Contrasena)) {
            const token = generateToken(bibliotecario);
            return res.status(200).send({ message: 'Login realizado exitosamente como bibliotecario', token });
          }
        }
...
const rolePermissions = {
  lector: ['read_books', 'view_profile'],
  bibliotecario: ['read_books', 'manage_books', 'view_profile', 'manage_users']
};
...
app.post('/noticias', (req, res) => {
  const { titulo, contenido, imagen, latitud, longitud } = req.body;
  console.log('Latitud:', this.latitud, 'Longitud:', this.longitud); // Verifica que los datos se reciban correctamente

  // Verificar que los campos no estén vacíos
  if (!titulo || !contenido || latitud === undefined || longitud === undefined) {
      return res.status(400).send({ message: 'Título, contenido, latitud y longitud son obligatorios' });
  }

  const query = 'INSERT INTO Noticias (Titulo, Contenido, Imagen, Latitud, Longitud) VALUES (?, ?, ?, ?, ?)';
    pool.query(query, [titulo, contenido, imagen, latitud, longitud], (err) => {
      if (err) {
